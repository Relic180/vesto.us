...
// TODO: This class still has LOTS of formatting cleanup work to do

const defaults = {
        'tagName': 'ul'
    },
    optionsRequiringRerendering = [
        'collection',
        'modelView',
        'modelViewOptions',
        'itemTemplate',
...
// This class is a rafactored and slightly extended version of Backbone.collectionView lib - https://rotundasoftware.github.io/backbone.collectionView/
export default class CollectionView extends View {
    constructor(options = {}) {
        ...
        this.initializationOptions = [ // Required to be array an of objects to work properly
            {'collection': null},
            {'modelView': null},
            {'modelViewOptions': {}},
            {'itemTemplate': null},
        ...
        // Easier than refactoring this entire 3rd party class to use this.options instead
        ...
        if (!this.collection) {
            this.collection = new Class.Collection.Base();
        }

        this.hasBeenRendered = false;

        this.$el.data('view', this); // needed for connected sortable lists
        this.$el.addClass('collection-list-inner');

        if (this.selectable) {
            this.$el.addClass('is-selectable');
        }
        ...
        this.listenTo(this, 'contentFiltered add remove reset', _.debounce(this.setVisibleIndices, 50).bind(this));
        this.setVisibleIndices();
    }
    ...
    render() {
        ...
        oldViewManager.each((thisModelView) => {
        ...
        this.collection.each((thisModel) => {
            let thisModelView = oldViewManager.findByModelCid(thisModel.cid);
            if (!this.reuseModelViews || _.isUndefined( thisModelView)) {
                // if the model view has not already been created on a previous render then create and initialize it now.
                thisModelView = this.createNewModelView(thisModel, this.getModelViewOptions(thisModel));
            }

            this.insertAndRenderModelView(thisModelView, fragmentContainer || modelViewContainerEl);
        });
        ...
        this.showEmptyListCaptionIfAppropriate();

        //this.trigger('render'); // Don't think we need or use this
    ...
    // To clear a filter, pass the filter key in the "remove" prop of the filterObj, or pass the key along with an additional property of "remove = true"
    applyFilter(filterObj) {
        ...
        _.defaults(filterObj, {
            affectAccess: true // Differentiate between content filtered by intent, and content filtered to improve performance
        });
    ...
    getFilteredSet() {
        if (!this.filters) return this.collection.models;
    ...
    setVisibleIndices() {
        let visibleModels = this.getFilteredSet();
    ...
    onOptionsChanged(changedOptions, originalOptions) {
        var rerender = false;

        _.each( _.keys( changedOptions ), ( changedOptionKey ) => {
            ...
            switch( changedOptionKey ) {
                case 'collection' :
                    ...
                    break;
                case 'selectMultiple' :
                    if( ! newVal && this.selectedItems.length > 1 )
                ...
                case 'selectable' :
                    ...
                    break;
                case 'sortable' :
                    changedOptions.sortable ? this.setupSortable() : this.$el.sortable( 'destroy' );
                    break;
                ...
                case 'processKeyEvents' :
                    if( newVal ) this.$el.attr( 'tabindex', 0 ); // so we get keyboard events
                    break;
                case 'modelView' :
                    //need to remove all old view instances
                    this.viewManager.each( ( view ) => {
                        this.viewManager.remove( view );
                        // destroy the View itself
                        view.remove();
                    } );
                    break;
            }
            if( _.includes( optionsRequiringRerendering, changedOptionKey ) ) rerender = true;
        } );

        if( this.hasBeenRendered && rerender ) {
            this.render();
        }
    }

    setOption(optionName, optionValue) { // now is mearly a wrapper around backbone.viewOptions' setOptions()
        var optionHash = {};
        optionHash[ optionName ] = optionValue;
        this.setOptions( optionHash );
    }

    getSelectedModel(options) {
        return this.selectedItems.length ? _.first( this.getSelectedModels( options ) ) : null;
    }

    getSelectedModels (options) {
        options = _.extend( {}, {
            by : 'model'
        }, options );
        ...
        switch( referenceBy ) {
            case 'id' :
                _.each( this.selectedItems, ( item ) => {
                    items.push( this.collection.get( item ).id );
                } );
                break;
            case 'cid' :
            ...
            case 'model' :
                _.each( this.selectedItems, ( item ) => {
                    items.push( this.collection.get( item ) );
                } );
                break;
            case 'view' :
                _.each( this.selectedItems, ( item ) => {
                    items.push( this.viewManager.findByModel( this.collection.get( item ) ) );
                } );
                break;
            default :
    ...
    setSelectedModels(newSelectedItems, options) {
        ...
        switch( referenceBy ) {
            ...
            case 'id' :
                this.collection.each( ( thisModel ) => {
                    if( _.includes( newSelectedItems, thisModel.id ) ) newSelectedCids.push( thisModel.cid );
                } );
                break;
            case 'model' :
                newSelectedCids = _.map( newSelectedItems, 'cid' );
                break;
            case 'view' :
                _.each( newSelectedItems, ( item ) => {
            ...
            case 'offset' :
                ...
                itemElements.each( (i, el) => {
                    var thisItemEl = $(el);
                    if( _.includes( newSelectedItems, curLineNumber ) )
    ...
    setSelectedModel(newSelectedItem, options) {
    ...
    showEmptyListCaptionIfAppropriate() {
        ...
        if( this.emptyListCaption ) {
            ...
            if( visibleEls.length === 0 ) {
                ...
                if( this._isRenderedAsList() )
                    $emptyListCaptionEl = $varEl.wrapAll( '<li class="not-sortable"></li>' ).parent().addClass('is-empty-caption');
                else
                    $emptyListCaptionEl = $varEl.wrapAll( '<tr class="not-sortable"><td colspan="1000"></td></tr>' ).parent().parent().addClass('is-empty-caption');

                this.getContainerEl().append( $emptyListCaptionEl );
            }
        }
    }

    _removeEmptyListCaption() {
        if( this._isRenderedAsList() )
            this.getContainerEl().find( '> li > var.empty-list-caption' ).parent().remove();
        else
            this.getContainerEl().find( '> tr > td > var.empty-list-caption' ).parent().parent().remove();
    }
    ...
    insertAndRenderModelView(modelView, parentElOrDocumentFragment, atIndex) {
    ...
    reapplyFilter(whichFilter) {
        if( ! _.includes( [ 'selectableModels', 'sortableModels', 'visibleModels' ], whichFilter ) ) {
            throw new Error( 'Invalid filter identifier supplied to reapplyFilter: ' + whichFilter );
        }

        switch( whichFilter ) {
            case 'visibleModels':
                this.viewManager.each(( thisModelView ) => {
                    var notVisible = this.visibleModelsFilter && ! this.visibleModelsFilter.call( this, thisModelView.model );

                    thisModelView.$el.toggleClass( 'not-visible', notVisible );
                    if( this._modelViewHasWrapperLI( thisModelView ) ) {
            ...
            case 'sortableModels':
                ...
                this.viewManager.each(( thisModelView ) => {
                    var notSortable = this.sortableModelsFilter && ! this.sortableModelsFilter.call( this, thisModelView.model );
            ...
            case 'selectableModels':
                this.viewManager.each(( thisModelView ) => {
                    var notSelectable = this.selectableModelsFilter && ! this.selectableModelsFilter.call( this, thisModelView.model );
    ...
    _removeModelView(modelView) {
        ...
        if( this.selectable ) this.restoreSelection();

        this.showEmptyListCaptionIfAppropriate();
    }

    _validateSelectionAndRender() {
        this._validateSelection();
        this.render();
    }
    ...
    registerCollectionEvents() {
        this.listenTo( this.collection, 'add', ( model ) => {
            ...
            if( this.hasBeenRendered ) {
                modelView = this.createNewModelView( model, this.getModelViewOptions( model ) );
                this.insertAndRenderModelView( modelView, this.getContainerEl(), this.collection.indexOf( model ) );
            }

            this.trigger( 'add', modelView );
        } );
        ...
        this.listenTo( this.collection, 'sort', ( collection, options ) => {
            if( this.hasBeenRendered && ( options.add !== true || this.forceRerenderOnNextSortEvent ) ) this.render();
            this.trigger( 'sort' );
        } );
    }
    ...
    _addSelectedClassToSelectedItems(oldItemsIdsWithSelectedClass) {
        ...
        _.each( itemsIdsFromWhichSelectedClassNeedsToBeRemoved, ( thisItemId ) => {
            this.getContainerEl().find( '[data-model-cid=' + thisItemId + ']' ).removeClass( 'selected' );

            if( this._isRenderedAsList() ) {
                this.getContainerEl().find( 'li[data-model-cid=' + thisItemId + '] > *' ).removeClass( 'selected' );
            }
        }, this );
        ...
        _.each( itemsIdsFromWhichSelectedClassNeedsToBeAdded, ( thisItemId ) => {
            this.getContainerEl().find( '[data-model-cid=' + thisItemId + ']' ).addClass( 'selected' );

            if( this._isRenderedAsList() ) {
                this.getContainerEl().find( 'li[data-model-cid=' + thisItemId + '] > *' ).addClass( 'selected' );
            }
        }, this );
    }

    _reorderCollectionBasedOnHTML() {
        this.getContainerEl().children().each(() => {
            ...
            if( thisModelCid )
            {
                ...
                var thisModel = this.collection.get( thisModelCid );
                if( thisModel )
                {
                    this.collection.remove( thisModel, { silent : true } );
                    this.collection.add( thisModel, { silent : true, sort : ! this.collection.comparator } );
                }
            }
        } );
    ...
    _getModelViewConstructor(thisModel) {
        return this.modelView || Class.View.Base;
    }

    getModelViewOptions(thisModel) {
    ...
    createNewModelView(model, modelViewOptions) {
    ...
    _wrapModelView(modelView) {
        ...
        if( _.isFunction( this.sortableModelsFilter ) )
            if( ! this.sortableModelsFilter.call( this, modelView.model ) ) {
                modelViewWrapperEl.addClass( 'not-sortable' );
                modelView.$el.addClass( 'not-selectable' );
            }

        if( _.isFunction( this.selectableModelsFilter ) )
            if( ! this.selectableModelsFilter.call( this, modelView.model ) ) {
    ...
    _convertStringsToInts(theArray) {
        return _.map( theArray, ( thisEl ) => {
    ...
    _containSameElements(arrayA, arrayB) {
    ...
    _modelViewHasWrapperLI(modelView) {
    ...
    _getVisibleItemEls() {
        var itemElements = [];
        itemElements = this.getContainerEl().find( '> [data-model-cid]:not(.not-visible)' );
    ...
    setupSortable() {
        var sortableOptions = _.extend( {
            ...
            change :  this._sortChange.bind(this),
    ...
    _sortStop(event, ui) {
        var modelBeingSorted = this.collection.get( ui.item.attr( 'data-model-cid' ) );
        var modelViewContainerEl = this.getContainerEl();
    ...
    _receive(event, ui) {
        ...
        var newIndex = this.getContainerEl().children().index( ui.item );
    ...
    _over(event, ui) {
        ...
        this.getContainerEl().find( '> var.empty-list-caption' ).hide();
    }
    ...
    _listItem_onMousedown(theEvent) {
        ...
        if( clickedItemId )
        {
            ...
            if( this.selectMultiple && theEvent.shiftKey )
            {
                ...
                if( this.selectedItems.length > 0 )
                {
                    this.collection.find( ( thisItemModel ) => {
                        firstSelectedItemIndex++;

                        // exit when we find our first selected element
                        return _.includes( this.selectedItems, thisItemModel.cid );
                    }, this );
                }
            ...
            else if( ( this.selectMultiple || _.includes( this.selectedItems, clickedItemId ) ) && ( this.clickToToggle || theEvent.metaKey || theEvent.ctrlKey ) )
            {
                if( _.includes( this.selectedItems, clickedItemId ) )
    ...
    _listItem_onDoubleClick(theEvent) {
        var clickedItemId = this._getClickedItemId( theEvent );
    ...
    _listBackground_onClick(theEvent) {
        if( ! this.selectable || ! this.clickToSelect ) return;
...
Backbone.ViewOptions.add = function(view, optionsDeclarationsProperty) {
    if (_.isUndefined(optionsDeclarationsProperty)) {
        optionsDeclarationsProperty = 'options';
    }

    // ****************** Public methods added to view ******************

    view.setOptions = function(options) {
        let optionsThatWereChanged = {},
            optionsThatWereChangedPreviousValues = {},
            optionDeclarations = _.result(this, optionsDeclarationsProperty);

        if (!_.isUndefined(optionDeclarations)) {
            let normalizedOptionDeclarations = _normalizeOptionDeclarations(optionDeclarations);

            _.each(normalizedOptionDeclarations, (thisOptionProperties, thisOptionName) => {
                let thisOptionRequired = thisOptionProperties.required,
                ...
                if (thisOptionRequired) {
                    ...
                    if ((!options || !_.includes(_.keys(options), thisOptionName)) && _.isUndefined(this[thisOptionName])) {
                        throw new Error(`Required option '${thisOptionName}' was not supplied.`);
                    }

                    if (options && _.includes(_.keys(options), thisOptionName) && _.isUndefined(options[thisOptionName])) {
                ...
                if (options && thisOptionName in options && !_.isUndefined(options[thisOptionName])) {
                    let oldValue = this[thisOptionName],
                        newValue = options[thisOptionName];
                ...
                } else if (_.isUndefined(this[thisOptionName])) {
                    // note defaults do not write over any existing properties on the view itself.
                    this[thisOptionName] = thisOptionDefaultValue;
                }
            });
        }

        if (_.keys(optionsThatWereChanged).length > 0) {
            if (_.isFunction(this.onOptionsChanged)) {
                this.onOptionsChanged(optionsThatWereChanged, optionsThatWereChangedPreviousValues);
            } else if (_.isFunction(this.onOptionsChanged)) {
                this.onOptionsChanged(optionsThatWereChanged, optionsThatWereChangedPreviousValues);
            }
        }
    };
...
function _normalizeOptionDeclarations(optionDeclarations) {
    // convert our short-hand option syntax (with exclamation marks, etc.) to a simple array of standard option declaration objects.
    let normalizedOptionDeclarations = {};

    if (!_.isArray(optionDeclarations)) throw new Error('Option declarations must be an array.');

    _.each(optionDeclarations, (thisOptionDeclaration) => {
        let thisOptionName,
        ...
        if (_.isString(thisOptionDeclaration)) {
            ...
            throw new Error('Each element in the option declarations array must be either a string or an object.');
        }
        ...
        if (!_.isUndefined(thisOptionDefaultValue)) {
            normalizedOptionDeclarations[thisOptionName].defaultValue = thisOptionDefaultValue;
        }
    });
...
class ChildViewContainer {
    constructor(views) {
        ...
        _.each(['forEach', 'each', 'map', 'find', 'detect', 'filter',
                'select', 'reject', 'every', 'all', 'some', 'any', 'include',
                'contains', 'invoke', 'toArray', 'first', 'initial', 'rest',
                'last', 'without', 'isEmpty', 'pluck'],
        (method) => {
            this[method] = function() {
                let views = _.values(this._views),
    ...
    findByModelCid(modelCid) {
        let viewCid = this._indexByModel[modelCid];
        return this.findByCid(viewCid);
    }

    // Find a view by a custom indexer.
    findByCustom(index) {
        let viewCid = this._indexByCustom[index];
        return this.findByCid(viewCid);
    }
    ...
    remove(view) {
        ...
        // delete custom index
        _.any(this._indexByCustom, (cid, key) => {
            if (cid === viewCid) {
